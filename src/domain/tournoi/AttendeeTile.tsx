...
export const AttendeeTile: React.FC<Props> = ({ data }) => (
  <div className="relative flex w-80 flex-col items-center rounded-lg border-2 border-gray-600 bg-blue-200 p-4">
    ...
    <img
      className="h-60 w-60 rounded-xl border-2 border-gray-400   object-cover object-center"
    ...
    <div className="z-30 font-bold">
      ...
      <div className="">Niveau actuel : {data.currentElo}</div>
      <div className="">PeakElo : {data.comment}</div>
      <div className="">Rôle : {data.role}</div>
      <div className="">Piscine de champions : {data.championPool}</div>
      <div>Lieu de naissance : {data.birthPlace}</div>
    </div>
    {/* <Image
        src={getImageUrl(data.avatar, 'attendees', data.id)}
        width={200}
